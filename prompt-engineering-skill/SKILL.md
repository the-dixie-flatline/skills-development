...
description: Family-specific prompt-engineering references for current-generation LLMs — Claude, GPT-5.x / o-series, gpt-oss, Gemini, Gemma, Llama, Qwen, Grok, Mistral, DeepSeek, GLM, MiniMax, Kimi. Trigger when the user writes or debugs prompts for a specific model family, configures sampling or thinking/reasoning budgets, picks a model variant, works with a family's chat template or tool-use protocol, migrates a prompt between versions, authors one prompt for multiple families or a family not covered here (portable-baseline lane), configures a hosted deep-research or agent-orchestration surface, prompts Gemini Notebook (formerly NotebookLM), calls a non-OpenAI family through an OpenAI-compatible endpoint, or reasons about consumer/web-UI vs API behavior. Skip for general prompt-engineering methodology beyond the structural baseline, and for how this current Claude session should respond.
version: 0.9.0
...
|---------|--------------|-----------|--------------------|
| Claude  | published    | published | Fable 5 flagship + **Opus 5** (`claude-opus-5`, launched 2026-07-24: recommended starting model, ~Fable-5 intelligence at half price, thinking on by default with disable-only-at-effort<=high 400 rule, classifiers incl. new `general_harms` category, cyber ~85% less restrictive than Fable 5, 512-token cache min, May 2026 cutoff, fast mode `speed:"fast"` $10/$50) + Sonnet 5 + Opus 4.8/4.7 Active + Haiku 4.5, as of 2026-08-09. Raw CoT now documented never-returned on ANY model; fallback headers rev'd to 2026-07-01 (+"default" mode); forced tool use works with adaptive thinking; Opus 4.1 retired 2026-08-05 (executed); **Mythos Preview 2026-07-21 retirement did NOT execute** (deprecated, undated); Sonnet 5 intro pricing ends 2026-08-31 |
...
| Gemini  | published    | published | **3.6 Flash GA 2026-07-21** (new Flash-tier lead: default thinking On (medium), ~17% fewer output tokens than 3.5 Flash, $1.50/$7.50, Mar 2026 cutoff, computer use as built-in tool Preview) + **3.5 Flash-Lite GA** (claimed 350 tok/s, $0.30/$2.50, replacement for 3.1 Flash-Lite) + 3.5 Flash GA + 3.1 Pro Preview (3.5 Pro partner-only; Gemini 4 pre-training started). **2.5-GA 2026-10-16 sunset WITHDRAWN** — deprecations page now shows "No shutdown date announced" for all three 2.5 models. Interactions API primary (GA 2026-06-22); `generateContent` legacy-but-supported (3.6 Flash ships on both). As of 2026-08-09. **Gemini Notebook is a separate surface** — see `gemini-notebook-surface.md`; these two files do not apply to it |
...
| DeepSeek| published    | published | **V4-Flash-0731** now serves `deepseek-v4-flash` (official release 2026-07-31, same API ID; April launch retroactively "Preview"; pro still Preview build), as of 2026-08-09. Thinking default-ON at effort high; effort ladder low/high/max real on flash; thinking mode ignores temperature/top_p/penalties; native Responses API (flash-only, stateless, server-side web_search); USD pricing $0.14 miss / $0.0028 hit / $0.28 out + **pre-announced significant price increase**; kv-cache guide rewritten (prefix units, 64-token min gone); legacy chat/reasoner retired 2026-07-24 (executed); docs moved (function_calling→tool_calls, reasoning_model→thinking_mode). Field-observed session guidance added: contract placement system-vs-first-user indistinguishable, no drift/recall loss to 237K ctx, effort switches cache-safe but thinking-toggle busts cache, CoT exhaustion = silent empty content. MIT, 1M ctx |
...
| Kimi    | published    | published | Kimi K3 (`kimi-k3`) flagship, 2.8T total / 104B active + KDA, 1M ctx, always-on reasoning with `reasoning_effort` **{low,high,max} default max** (widened from max-only; mid-session switch busts prefix cache), as of 2026-08-09. **Open weights released ~2026-07-27** (`moonshotai/Kimi-K3`, custom Kimi K3 License: MIT-style + $20M MaaS threshold + 100M-MAU attribution; vLLM 0.27+/SGLang/TokenSpeed, 8x GB300-class floor). `tool_choice: required` K3-exclusive; vendor-documented limitations: excessive proactiveness, thinking-history-replay sensitivity; web-search caution now vendor-inconsistent (disputed). K3 unlock needs $1 top-up. K2.5/moonshot-v1 sunset 2026-08-31 on schedule; Active K2.x: k2.6 (256K), k2.7-code(-highspeed) |
...
- **Reasoning / thinking controls are not portable.** Parameter names, accepted values, and semantics all differ: `reasoning.effort` {none,low,medium,high,xhigh} (OpenAI); `thinking.type: "adaptive"` + `output_config.effort` (Claude — adaptive-only on Opus 4.7/4.8 and Fable 5; on Fable 5 thinking is always on and `disabled` is unsupported; on Opus 5 thinking is on by default and `disabled` is accepted only at effort high or below, 400 with xhigh/max; manual budgets rejected); `thinkingLevel` {minimal,low,medium,high} on Gemini 3 (3.6 Flash default On/medium) or `thinkingBudget` on Gemini 2.5; `enable_thinking` + `preserve_thinking` (Qwen); `<|think|>` token (Gemma); `thinking: {type:"enabled"|"disabled"}` or `reasoning_effort` {low,high,max} — thinking ON by default at high; low is flash-only-real, pro maps low→high, xhigh→max (DeepSeek V4, as of 2026-08-09); binary `reasoning_effort: "high" | "none"` (Mistral, chat endpoint); `reasoning_effort` {low,medium,high} default `high`, no `none` (Grok grok-4.5); {none,low,medium,high} default `low` (Grok grok-4.3); `thinking.type: enabled|disabled` plus a 7-value `reasoning_effort` {none,minimal,low,medium,high,xhigh,max} that shims onto two tiers, GLM-5.2+ only (GLM); `thinking: {type:"adaptive"|"disabled"}`, M3 default OFF, M2.x cannot disable (MiniMax); always-on with `reasoning_effort` {low,high,max} default `max` — widened from max-only, and switching it mid-session invalidates the prefix cache (Kimi K3, as of 2026-08-09); `reasoning_effort` {low,medium,high} default medium set *inside the system message*, not a top-level param (gpt-oss / gpt-oss-safeguard, Harmony-format open weights); nothing at all (Llama). Treat `reasoning_effort` as a family-scoped name, not a universal knob — even where the name is shared, the accepted value set differs.
